...
void triRenderHelper(
    ...
    // sice a -> b -> c is clockwise, we know that a[1] > b[1].
        ...
        while (x[0] <= floor(c[0]) && x[0] <= buf->width){
            x[1] = ceil(b[1] + (c[1]-b[1])/(c[0]-b[0])*(x[0]-b[0]));
            if(x[1] < 0)
                x[1] = 0;
            while (x[1] <= floor(a[1] + (c[1]-a[1])/(c[0]-a[0])*(x[0]-a[0])) && x[1] <= buf->height) {
                setPixel(sha, buf, unif, tex, x, a, invertedItpCoeffs, bMinusA, cMinusA);
                x[1] = x[1] + 1;
            }
            x[0] = x[0] + 1;
    ...
    // o.w., c[0] < b[0], so loop from a[0] to b[0] and reframe checks around c[0].
        ...
        while (x[0] <= floor(b[0]) && x[0] <= buf->width){
            ...
            if(x[1] < 0)
                x[1] = 0;
            while (x[1] <= floor(b[1] + (c[1]-b[1])/(c[0]-b[0])*(x[0]-b[0])) && x[1] <= buf->height) {
                setPixel(sha, buf, unif, tex, x, a, invertedItpCoeffs, bMinusA, cMinusA);
                x[1] = x[1] + 1;
            }
            x[0] = x[0] + 1;
