/*
    110triangle.c
    C file to rasterize a given triangle and render it with interpolated and modulated color.
...
void setPixelColor(const double x[2], const double a[2], const double invertedItpCoeffs[2][2], const double alpha[3], const double betaMinusAlpha[3], const double gammaMinusAlpha[3], const double rgb[3]) {
    // variables which depend on the position of x, and therefore need to be calculated every time setPixelColor() is called.
    double xMinusA[2];
    double pq[2];
    double smallVec1[3]; // represents p(beta - alpha)
    double smallVec2[3]; // represents q(gamma - alpha)
    double smallVecSum[3]; // represents smallVec1 + smallVec2
    double chi[3];
    ...
    vecScale(3, pq[0], betaMinusAlpha, smallVec1);
    vecScale(3, pq[1], gammaMinusAlpha, smallVec2);
    vecAdd(3, smallVec1, smallVec2, smallVecSum);
    vecAdd(3, alpha, smallVecSum, chi);
    
    // modulates the interpolated color with the passed-in rgb color.
    vecModulate(3, chi, rgb, rrggbb);
    ...
    if (a[0] == c[0]) {
        while (x[0] <= floor(b[0])){
            ...
            while (x[1] <= floor(c[1] + (b[1]-c[1])/(b[0]-c[0])*(x[0]-c[0]))) {
                setPixelColor(x, a, invertedItpCoeffs, alpha, betaMinusAlpha, gammaMinusAlpha, rgb);
                x[1] = x[1] + 1;
            }
            x[0] = x[0] + 1;
        }
