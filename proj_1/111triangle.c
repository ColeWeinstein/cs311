/*
    111triangle.c
    C file to rasterize a given triangle and render it with interpolated and modulated color. (Old version of current 110.triangle, without setPixelColor() function.)
...
void triRenderHelper(const double a[2], const double b[2], const double c[2], const double rgb[3], const double alpha[3], const double beta[3], const double gamma[3]) {
    ...
    // these arrays are used later, but I felt like it would be more efficient to just declare them here.
    double pq[2];
    ...
    double chi[3];
    double rrggbb[3]; // chi[] modulated with rgb[]
    ...
    if (a[0] == c[0]) {
        while (x[0] <= floor(b[0])){
            ...
            while (x[1] <= floor(c[1] + (b[1]-c[1])/(b[0]-c[0])*(x[0]-c[0]))) {
                ...
                pixSetRGB(x[0], x[1], rrggbb[0], rrggbb[1], rrggbb[2]);
                x[1] = x[1] + 1;
            }
            x[0] = x[0] + 1;
        }
