/*
    271triangle.c
    C file to rasterize a given triangle and render it. triRender and its subcalls interpolate the varyings passed into it, then invoke sha->shadeFragment for a fragment color and depth (after any number and type of artistic transformations). Only set pixel if fragment is the closest fragment to screen so far.
    ...
    Here, the check happens in setPixel() (so that only one line is written). In 270triangle.c, the check happens in triRenderHelper() (so that setPixel() is only called when necessary). This file is generally slower and causes freezes, but appears somewhat simpler/cleaner.
